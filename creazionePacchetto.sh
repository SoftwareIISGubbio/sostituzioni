#!/bin/bash
# ATTENZIONE: questo script fa affidamento sulla variabile JAVA_HOME
# sulla struttura delle cartelle del progetto e su quelle create da maven

# la costruzione del pacchetto è piuttosto "manuale", 
# sul sito di javafx ci sono anche indicazioni per maven.
...
JAR_PRINCIPALE=$(cd $CARTELLA_JARS; ls sostituzioni*.jar)
# cartella di lavoro
...
$JPACKAGE --dest $DESTINAZIONE --type $TIPO_PACCHETTO -n sostituzioni --icon $ICONA --input $CARTELLA_LAVORO \
    --add-modules javafx.controls,javafx.media,javafx.fxml,jdk.charsets --module-path $CARTELLA_LAVORO \
