...
/**
* questo filtro serve per trovare tutti i docenti con l'ora buca nell'ora ricercata
* @author Nicola
*/
	...
	*/
	public static ArrayList<Docente> docentiOreBuche(ArrayList<Docente> tutti, Ora oreBuche) {
		...
		for (Docente d : tutti) {
			...
			//definisco OraMenoUno che equivale all'ora prima dell'ora cercata per verificare che sia un ora buca
			Ora OraMenoUno = new Ora(ora.giorno, ora.orario-1);
			//definisco OraMenoDue che equivale a 2 ore prima dell'ora cercata per verificare che sia un ora buca
			Ora OraMenoDue = new Ora(ora.giorno, ora.orario-2);
			//definisco OraPiuUno che equivale all'ora dopo dell'ora cercata per verificare che sia un ora buca
			Ora OraPiuUno = new Ora(ora.giorno, ora.orario+1);
			//definisco OraPiuDue che equivale a 2 ore dopo dell'ora cercata per verificare che sia un ora buca
			...
			if (!d.lavoraNellOra(ora)) { 
				...
				//controllo se lavora 2 ore prima e l'ora dopo
				if(d.lavoraNellOra(OraMenoDue) && d.lavoraNellOra(OraPiuUno)) {
					risposta.add(d);
				}
				//controllo se lavora l'ora prima e 2 ore dopo
				if(d.lavoraNellOra(OraMenoUno) && d.lavoraNellOra(OraPiuDue)) {
					risposta.add(d);
				}
			}
